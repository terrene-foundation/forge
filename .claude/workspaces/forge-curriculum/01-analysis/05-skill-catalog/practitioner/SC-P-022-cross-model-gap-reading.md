...
journal_evidence:
  - path: loom/journal/0049-DISCOVERY-coc-token-ablation-experiment.md
    polarity: NEGATIVE
    quote: "MiniMax needs MORE rules than Claude, not fewer: Plaintext password comparison (row[0] == password) — critical security flaw. f-string around sql.Identifier() — knows the tool, uses it wrong. Hardcodes API keys in code examples."
...
- SC-P-002 (model-vs-rule ablation) — the empirical methodology that produces the per-model gap profiles this atom requires
