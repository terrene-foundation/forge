...
journal_evidence:
  - path: loom/journal/0002-DECISION-cc-expert-artifacts.md
    polarity: POSITIVE
    quote: "Created 4 new artifact types: Agent (4-dimension audit), Skill (SKILL.md + artifact-design.md + anti-patterns.md + token-budget.md), Rule (path-scoped, 6 MUST + 4 MUST NOT with DO/DO NOT examples), Command (/cc-audit launches systematic artifact audit)."
  ...
  - path: loom/journal/0015-DECISION-version-tracking-cascade.md
    polarity: POSITIVE
    quote: ".claude/VERSION (JSON) in every repo: own version + upstream version + changelog. version-utils.js in hooks/lib/: reads VERSION, fetches upstream via curl (3s timeout), compares."
