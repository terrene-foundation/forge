...
- SC-A-012 (CLAUDE.md as Layer 2 context) — the master directive is the primary baked-in artifact; its size directly affects the token budget available for conversation.
- SC-P-022 (cross-model gap reading) — multi-model envelopes drive the variant architecture decision; weaker models need full rule sets where stronger models can ship compressed.
