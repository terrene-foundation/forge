...
- SC-A-004 (Layer 3 hook security audit) — hooks that manipulate signed structs must respect encapsulation; a hook with elevated privilege bypassing private fields is worse than application code doing the same.
...
- SC-A-007 (zero-config security audit) — zero-config constructors that default signed-struct fields to permissive values (e.g., `dimension_scope = None` meaning "all dimensions") must be audited for the same encapsulation violations.
- SC-P-010 (timing race condition red team) — timing attacks on signed structs (e.g., signature verification that leaks key length) are a cross-cutting security concern that encapsulation alone does not prevent.
