...
Decompose any multi-session feature into milestones ordered by dependency graph, not by sequence number. For each milestone, name the approval gate and the specific evidence required to pass it (not "done" or "tests pass" but concrete acceptance criteria). Identify which milestones can run in parallel, which must wait, and where the convergence point is. Make gates hard-blocking: the next milestone cannot start until the gate evidence is produced and reviewed.
