...
For every convenience API that creates network-accessible resources, enumerate the full exposure surface: what endpoints are created, which accept writes, whether they require authentication, and whether they bind to localhost or all interfaces. Treat the zero-config path as the highest-risk path in the threat model, not the simplest. The resolution pattern is: localhost binding + writes disabled by default + production requiring explicit auth, preserving the one-line usability while closing the security gap.
...
- SC-A-003 (defense-in-depth codification) — zero-config paths are the enforcement surface most likely to be missing from a defense-in-depth stack because they bypass explicit configuration entirely.
- SC-A-004 (Layer 3 hook security audit) — hooks are one enforcement mechanism for the guardrails this atom identifies as missing; hooks invoked during zero-config startup inherit the permissive defaults.
- SC-A-005 (encapsulation as security boundary) — zero-config constructors that default signed-struct fields to permissive values create the same bypass risk as public fields on governed structs.
...
- SC-P-010 (timing race condition red team) — zero-config paths that skip explicit auth setup may also skip constant-time comparison, introducing timing side-channels alongside the permission gap.
