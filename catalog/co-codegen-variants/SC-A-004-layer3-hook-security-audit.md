...
Include Layer 3 hooks in every red team pass. For each hook, trace every external input (files, environment variables, git state) to its consumption point and classify as safe or unsafe. Prioritise three vulnerability classes: (1) shell injection via `execSync` with untrusted input, (2) path traversal via unsanitized file paths, (3) denial of service via unbounded reads or missing timeouts. The standing checklist item is: "Have the Layer 3 hooks been audited for the same vulnerability classes they enforce against?"
...
- SC-A-005 (encapsulation as security boundary) — hooks that manipulate signed structs or governance records must respect encapsulation boundaries; a hook with shell access bypassing private fields is a privilege escalation.
...
- SC-A-007 (zero-config security audit) — hooks invoked during zero-config startup inherit the permissive defaults of the zero-config path, making them higher-risk than hooks in explicitly configured environments.
- SC-P-010 (timing race condition red team) — timing side-channels in hooks (e.g., variable-time validation in session-start scripts) are a security concern that the hook audit should cover.
