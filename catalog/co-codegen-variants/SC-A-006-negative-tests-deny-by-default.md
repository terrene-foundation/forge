...
For every enforcement point with deny-by-default semantics, write at least one negative test that exercises the deny path with an input the enforcement has never seen (unmapped route, unknown role, NaN value). Write this test before the implementation, not after. The negative test asserts both the denial outcome (HTTP 403, rejected input, blocked action) and the logging of the denial reason. If existing enforcement code lacks negative tests, add them retroactively and treat any failures as critical findings.
