...
Before writing any new module or package, grep the existing codebase for primitives that already handle the core functionality. Record the search results explicitly: what exists, what is genuinely missing, and how the scope changes given what exists. The default outcome is composition with existing primitives, not creation from scratch. When the search finds multiple competing implementations, consolidate into one canonical primitive rather than adding another variant.
