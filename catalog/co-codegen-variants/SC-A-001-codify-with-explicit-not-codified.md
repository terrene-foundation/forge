...
journal_evidence:
  - path: loom/kailash-rs/workspaces/kailash-rl/journal/0003-DECISION-codify-phase0.md
    polarity: POSITIVE
    quote: "## What was NOT codified (and why)\n- **RL algorithm implementations**: These are code, not institutional knowledge. The source is the authority.\n- **Environment physics**: CartPole/Pendulum/MountainCar physics match Gymnasium — this is established science, not a decision to capture.\n- **Prior art search results**: Already captured in journal 0002-DISCOVERY."
...
- **0012-DECISION-codify-session7-ci-fixes** in kailash-py (POSITIVE) — The "one-off bug" reason in operation. Three items were excluded specifically because they were bugs, not patterns: SQLite cursor leak, `id_type.__name__` AttributeError, SQLite read-back. The entry illustrates the discrimination — three of the codified items were patterns (`__del__` super-call, `import warnings` inside `__del__`, PACT (primitives) bridge approval), and the three excluded items were bugs. Same session, same code base, the difference is whether the issue recurs and constrains future work.

## How to apply

Structure every codify session output with both a "What Was Codified" and a "What Was NOT Codified (and why)" section. For each excluded item, assign one of four reasons: (a) code-is-authority, (b) established science, (c) one-off bug, or (d) already captured at a specific path. If an exclusion does not fit one of these four, re-examine whether the exclusion is justified or whether the item should be codified after all.
...
- SC-A-003 (defense-in-depth codification) — complementary move: defense-in-depth is about breadth of landing across artifact types; NOT-codified is about precision of exclusion within each codify session.
- SC-P-017 (false-positive learning detection) — the explicit gap "/codify never reads from the learning system" is the kind of NOT-codified fact this atom teaches practitioners to surface.
