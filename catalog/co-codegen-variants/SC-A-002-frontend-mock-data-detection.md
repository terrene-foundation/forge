...
journal_evidence:
  - path: loom/journal/0047-DECISION-spec-coverage-gate.md
    polarity: NEGATIVE
    quote: "Post-mortem of a project (kz-engage) that passed 11 red team rounds, 94 tests, and 2 consecutive clean convergence rounds — yet shipped with:\n- Frontend pages using `generateHourlyOccupancy()` instead of real API calls [...]\n4. **Mock data invisible to hooks** — `validate-workflow.js` detects Python stubs but not `MOCK_USERS`, `generateHourly*()` in JS/TS"
...
Add frontend mock-data patterns (`MOCK_*`, `FAKE_*`, `DUMMY_*`, `SAMPLE_*`, synthetic `generate*()` / `mock*()` functions, `Math.random()` for display values) to your hook-layer validation alongside existing Python stub detection. Place the detection in a deterministic enforcement hook (e.g., `validate-workflow.js`), not only in advisory rules, so that enforcement is guaranteed regardless of whether the model reads the rule in a given session. When adding patterns, calibrate regex specificity to avoid false positives on legitimate utilities like `generateUUID()`.
...
- SC-A-003 (defense-in-depth codification) — the 0047 fix landed across six artifacts (rule + hook + three commands + skill); this is defense-in-depth codification applied to a detection gap.
- SC-A-006 (negative tests for deny-by-default) — once the hook blocks mock data patterns, negative tests must verify the deny path with real mock-data examples.
