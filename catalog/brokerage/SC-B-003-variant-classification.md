...
journal_evidence:
  ...
  - path: loom/journal/0048-DECISION-gate1-rs-variant-classification.md
    polarity: MIXED
    quote: "After thorough analysis of all 43 skill directories in kailash-rs/.claude/skills/, only 2 dirs needed upstreaming as new rs variants to loom"
...
- SC-B-010 (drift detection audit) — drift gaps surface when a variant-classified artifact in a USE repo disagrees with its global counterpart; the classification test resolves the disagreement.
- SC-B-016 (bidirectional cross-language parity) — the variant classification test is the single-file decision; cross-language parity is the cross-repo audit that checks whether the same classification was applied consistently across both SDKs.
- SC-P-002 (model-vs-rule ablation) — ablation data reveals which rules are model-native across all models (global) vs model-specific (variant), informing the classification test with empirical evidence.
