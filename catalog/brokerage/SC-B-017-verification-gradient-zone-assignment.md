...
- SC-B-002 (authority-chain escalation) — the zone assigned to the escalating agent determines whether the upstream fix proposal requires held approval or can proceed as flagged.
...
- SC-B-018 (PDP/PEP separation) — zone assignment determines the verification policy (PDP); the enforcement point (PEP) must match the assigned zone's strictness level.
- SC-A-013 (agent specialisation routing) — routing determines which specialist handles the work; zone assignment determines the trust level of that specialist's output.
