...
- SC-B-004 (specialist delegation) — specialists are the first to discover cross-SDK defects during framework-work review; their corrections list is the input to upstream brokerage.
...
- SC-B-016 (bidirectional cross-language parity) — parity audits reveal which SDK's implementation should be the upstream reference for shared abstractions.
- SC-B-015 (MUST rule reinterpretation) — when a rule in the consumer repo blocks the upstream-first approach, the reinterpretation move amends the rule to allow escalation.
