...
- SC-B-007 (worktree-per-agent) — independent red team agents need worktree isolation to prevent one agent's exploratory changes from contaminating another's findings.
- SC-P-003 (cross-feature interaction red team) — cross-feature interactions are a common source of convergent findings when independent agents probe different feature boundaries and discover the same integration gap.
