...
- SC-B-003 (variant classification) — orphan variants are often misclassified files that the classification test would have caught at authoring time.
- SC-B-010 (drift detection audit) — the audit that the parity check operationalises at the manifest level.
- SC-B-011 (version tracking cascade) — manifest parity should verify that version pins in distributed files match the cascade's expected values.
