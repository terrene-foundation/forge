...
- SC-B-003 (variant classification) — parity audits surface global-vs-variant questions: an abstraction that differs between SDKs may be a genuine variant (language-idiomatic) or an unintentional drift.
...
- SC-B-014 (explicit supersession journaling) — when a parity audit reverses the assumed reference direction, the prior assumption must be superseded explicitly in the journal.
