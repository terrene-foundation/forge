...
Sync/drift cluster: this atom is the operational core. Run alongside SC-B-010 (drift audit) before sync, SC-B-012 (manifest parity) at the gate, SC-B-003 (variant classification) at every collision, and SC-B-011 (version cascade) at session start.
...
- SC-B-010 (drift detection audit) — the pre-sync audit that produces the numbered drift-gap list this move halts on; without the audit, the per-file diff is reactive rather than proactive.
- SC-B-011 (version tracking cascade) — the version-mismatch check that runs before this move so that sync never propagates artifacts authored against a different SDK version.
- SC-B-012 (manifest parity automation) — the automated pre-flight check that this move depends on; manifest gaps must be resolved before per-file merge can begin.
- SC-B-013 (cross-workspace dependency mapping) — when sync writes span workspaces, dependency mapping is the prerequisite that prevents the merge from breaking shared bottleneck files.
- SC-B-014 (explicit supersession journaling) — when a sync resolves a drift by superseding an earlier decision, the supersession must be journaled at sync time, not after.
