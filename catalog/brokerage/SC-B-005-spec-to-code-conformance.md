...
PACT §40 (EATP Integration) requires that every PACT (spec) governance action creates a corresponding EATP record. CO §49 (Quality Criteria — Enforcement Reliability) demands that critical rules have hard enforcement, verified by intentionally attempting to violate them. PACT §10 (Monotonic Tightening Invariant) requires write-time rejection of envelope widening across all dimensions — not just the dimensions the implementer remembered. A conformance audit is the move that detects the gap between what the spec mandates and what the code actually enforces.

## Evidence walkthrough

- **kailash-py 0014** (POSITIVE) — Demonstrates the move executed well. The agent read PACT-Core-Thesis.md section by section, then audited the Python PACT (primitives) module against each normative requirement. Result: four numbered gaps (EATP record emission, write-time tightening missing three dimensions, compile_org silent drop, vacancy hard-block without interim envelope), each filed as a GitHub issue. The Rust SDK was already compliant on all four, proving the gaps were implementation omissions rather than spec ambiguities.
- **kailash-py 0015** (NEGATIVE) — Shows the cost of never performing this audit on the integration boundary. PACT (primitives) and EATP lived in the same `trust/` package tree with zero imports between them. The normative "EVERY governance action creates an EATP record" requirement was never wired. The entry names the specific absent call chain: GovernanceEngine never instantiates a TrustStore and never calls TrustOperations.
...
- SC-B-009 (cross-SDK upstream brokerage) — cross-SDK conformance gaps should be filed upstream rather than fixed only in the non-conformant SDK.
- SC-B-016 (bidirectional cross-language parity) — parity audits complement conformance audits; conformance checks code against spec, parity checks code against the other SDK's implementation.
- SC-P-014 (zero-tolerance scope discipline) — conformance gaps are failures (spec says MUST, code violates) not gaps (spec says MUST, code does not attempt); the distinction affects resolution priority.
