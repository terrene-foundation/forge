...
- SC-B-003 (variant classification) — version mismatches can cause a rule authored against a newer SDK to be misclassified when applied to a project on an older SDK.
...
- SC-B-012 (manifest parity automation) — manifest parity checks should include version-pin consistency as one of the parity dimensions.
