...
journal_evidence:
  - path: loom/kailash-py/workspaces/kailash/journal/0010-DISCOVERY-pypi-tag-push-race-condition.md
    polarity: NEGATIVE
    quote: "Session 5 claimed 4 packages were released to PyPI, but only kailash-kaizen 2.3.2 actually published. Three packages (kailash 2.3.0, kailash-dataflow 1.3.0, kaizen-agents 0.6.0) had tags created and pushed to GitHub but the `publish-pypi.yml` workflow never triggered for them."
...
- SC-A-003 (defense-in-depth codification) -- timing-sensitive enforcement requires multiple independent verification surfaces; a single constant-time comparison is insufficient if the surrounding iteration strategy negates it.
...
- SC-A-005 (encapsulation as security boundary) -- timing attacks on signed structs (e.g., signature verification that leaks information about key structure) are a cross-cutting security concern that encapsulation alone cannot prevent.
- SC-A-006 (negative tests for deny-by-default) -- enforcement mechanisms with deny semantics can be bypassed via timing; timing-aware negative tests are the verification methodology.
- SC-A-007 (zero-config security audit) -- zero-config paths that skip explicit auth setup may also skip constant-time comparison, compounding the exposure.
...
- SC-B-018 (PDP/PEP separation) -- timing attacks exploit the gap between PDP decision and PEP enforcement; if the two are separated, the gap must be minimized or made atomic.
