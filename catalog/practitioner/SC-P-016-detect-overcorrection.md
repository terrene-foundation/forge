...
Teaching sequence (rule-optimization cluster): **SC-P-002 → SC-P-016 (this) → SC-A-008 → SC-P-012 → SC-P-022**. Run ablation → detect overcorrection → classify rules → decide demote/strengthen → author for the weakest model.

- SC-P-002 (model-vs-rule ablation) — the upstream methodology this atom protects; clean-room isolation from SC-P-002 is the only reliable way to avoid the contamination this atom teaches you to detect.
- SC-A-008 (rule classification critical vs advisory) — the next step in the cluster: only after contamination is ruled out can ablation data be trusted for classification.
- SC-P-012 (rule demotion judgment) — demotion decisions depend on accurate baseline measurement, which depends on detecting overcorrection in both directions.
- SC-P-022 (cross-model gap reading) — the cluster's terminus: contamination detection must run against every model in the envelope, not just the strongest.
- SC-P-011 (audit categorization progressive disclosure) — sibling pattern: another case where the measurement method must match the thing being measured.
