...
journal_evidence:
  - path: loom/kailash-py/workspaces/kailash-align/journal/0007-DISCOVERY-trl-dependency-chain-fragility.md
    polarity: NEGATIVE
    quote: "A user installing kailash-align today with `trl>=0.8` could get TRL 0.12 (which lacks SFTConfig entirely), and AlignmentPipeline would fail at import time with an obscure ImportError."
...
- SC-B-011 (version tracking cascade) -- the version cascade must track SDK dependency pins; a dependency chain is only as strong as the weakest version pin in the cascade.
- SC-P-017 (false-positive learning detection) -- a broken learning pipeline is a dependency chain where the weakest link determines the value of the entire chain; the same fragility analysis applies.
