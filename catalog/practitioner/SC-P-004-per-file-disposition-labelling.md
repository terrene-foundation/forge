...
- SC-P-009 (architecture doc as contract) — disposition labelling is the file-level ground truth that architecture documents must be tested against.
- SC-P-021 (reference sweep after rename) — files labelled for rename or deletion in the disposition audit need proactive reference sweeps before implementation begins.
