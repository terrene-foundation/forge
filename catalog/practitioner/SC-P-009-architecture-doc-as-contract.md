...
- SC-P-013 (estimate self-contradiction) -- architecture documents that contain internally inconsistent claims (e.g., "5 capabilities missing" when 3 already exist) are a source of estimate contradictions that propagate into plans.
- SC-B-016 (bidirectional cross-language parity) -- the nexus-parity audit found that 60% of claimed-missing capabilities already existed; treating architecture docs as contracts catches this class of error before planning begins.
